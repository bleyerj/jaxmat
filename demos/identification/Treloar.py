...
loading_uni = ImposedLoading(
    "finite_strain",
    FXX=lamb_list,
    FYY=1 / jnp.sqrt(lamb_list),
    FZZ=1 / jnp.sqrt(lamb_list),
)
...
loading_equiax = ImposedLoading(
    "finite_strain", FXX=lamb_list, FYY=lamb_list, FZZ=1 / lamb_list**2
)
...
def eval_loss(results, stress_data):
    return sum(
        [
            jnp.mean(jnp.square(P1 - P1_data))
            for (P1, P1_data) in zip(results, stress_data)
        ]
    )
...
# scale_mat = None
scale_mat = jm.Hyperelasticity(jm.CompressibleOgden(mu=1.0, alpha=5.0, kappa=1e3))
...
print(alphas)
scaled_mat = jm.Hyperelasticity(jm.CompressibleOgden(mu=mus, alpha=alphas, kappa=1))
scaled_mat = jax.tree.map(
    lambda x: jnp.asarray(x, dtype=jnp.float64), scaled_mat
...
def loss(scaled_material, loadings):
    ...
    results = solve_hyperelasticity(material, loadings)
    return [
        (P1 - P1_data) / jnp.max(P1_data) for (P1, P1_data) in zip(results, P_)
    ], results
...
sol = optx.least_squares(
    loss, solver, new_material, loadings, has_aux=True, throw=False, max_steps=500
)
